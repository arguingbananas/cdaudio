...
`libchromaprint-tools` provides `fpcalc`, which is required for AcoustID
fingerprinting. For macOS, install the tools via Homebrew:
...
`--auto-release` uses AcoustID and MusicBrainz; you must supply an
AcoustID API key via `--acoustid-key` to enable automatic release matching.
...
Note: `--auto-release` requires network access and an AcoustID API key. The
project's `pyproject.toml` already defines the console script entry point:
